...
function Summery({enabledNext}) {
    ...
    const GenerateSummeryFromAI=async()=>{
        ...
        try{
            ...
            // Groq's json_object mode always returns an object, so the summary
            // array may come back wrapped (e.g. { summaries: [...] }) instead of
            // as a bare array - handle both shapes.
            const list=Array.isArray(parsed)?parsed:Object.values(parsed).find(v=>Array.isArray(v))||[];
            setAiGenerateSummeryList(list)
        }catch(error){
            console.error(error);
            toast.error(error.message || 'Failed to generate summary from AI');
        }finally{
            setLoading(false);
        }
    }
